defmodule Amazin.Pages.CartPage do
  ...
  side effects based on the outcome.
    ...
    case Checkout.validate(page.items) do
      {:ok, items} ->
        case Inventory.check_availability(items, stock) do
          ...
          {:error, unavailable} ->
            {page, {:error, {:out_of_stock, unavailable}}}
        end

      {:error, :empty_cart} ->
        {page, {:error, :empty_cart}}
    end
  end
  ...
  def handle(:checkout_failed, _data, page) do
    {%{page | checkout_status: :error}, {:checkout_error, "Checkout failed. Please try again."}}
  end
